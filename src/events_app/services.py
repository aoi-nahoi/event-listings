...
from .models import Bookmark, Category, Event
...
def seed_demo_content() -> None:
    if User.objects.exists() or Category.objects.exists() or Event.objects.exists():
        return
    categories = {
    ...
    users = [
        User.objects.create_user(username="sota", email="sota@example.com"),
        User.objects.create_user(username="mika", email="mika@example.com"),
        User.objects.create_user(username="ren", email="ren@example.com"),
    ]
    today = date.today()
    samples = [
